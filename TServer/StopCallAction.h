...
void stopCallAction(CRemotePeer* pRemote, const std::string& param, uint64_t callId, const std::string& type)
{
	...
	try{
		...
		std::string callCommand = CRpcJsonParser::mergeCommand("stopCall", callId, param.c_str());
		...
		//	std::string callJsonStr = CRpcJsonParser::buildCall("stopCall", clientCallId, args);
