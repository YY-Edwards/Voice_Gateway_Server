...
BOOL CUtil::DeleteDirectory(LPCTSTR pstrFolderPath)
{
	...
	_tcscpy_s(DirPath, pstrFolderPath);
	_tcscat_s(DirPath, _T("\\*"));    // searching all files
	_tcscpy_s(FileName, pstrFolderPath);
...
BOOL CUtil::SetApplicationAutoStart()
{
	...
	HKEY	hkey;
	long r = RegOpenKeyEx(HKEY_LOCAL_MACHINE, _T("Software\\Microsoft\\Windows\\CurrentVersion\\Run"),
