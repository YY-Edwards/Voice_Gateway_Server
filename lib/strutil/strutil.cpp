...
namespace strutil {
	...
	vector<wstring> split(const wstring& str, const wstring& delimiters) {
		vector<wstring> ss;

		TokenizerW tokenizer(str, delimiters);
		while (tokenizer.nextToken()) {
			ss.push_back(tokenizer.getToken());
		}
...
namespace strutil {
	const wstring TokenizerW::DEFAULT_DELIMITERS(L" \t\n\r");
	TokenizerW::TokenizerW( const std::wstring& str )
		: m_String(str), m_Offset(0), m_Delimiters(DEFAULT_DELIMITERS) {}

	TokenizerW::TokenizerW( const std::wstring& str, const std::wstring& delimiters )
		: m_String(str), m_Offset(0), m_Delimiters(delimiters) {}

	bool TokenizerW::nextToken()
	{
		return nextToken(m_Delimiters);
	}
	bool TokenizerW::nextToken(const std::wstring& delimiters) {
	...
	const std::wstring TokenizerW::getToken() const
	{
		return m_Token;
	}

	void TokenizerW::reset()
	{
		 m_Offset = 0;
	}
