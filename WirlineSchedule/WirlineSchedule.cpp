...
int main()
{
	...
	returnValue = manager.initSys(MASTER_IP, MASTER_PORT, LOCAL_PEER_ID, LOCAL_RADIO_ID, RECORD_TYPE, DEFAULT_SLOT, DEFAULT_GROUP, DONGLE_PORT);
	if (returnValue != 0)
	{
		printf_s("Initialization system fail\r\n");
		char temp = 0x00;
	...
	while (run)
	{
		...
		case 0x01:
		{
					 manager.initialCall(DEFAULT_GROUP, DEFAULT_CALLTYPE);
		}
			break;
		case 0x02:
		{
