...
static CMySQL g_db;
static CManager *g_pManager = new CManager(&g_db);
...
int main()
{
	...
	g_pManager->initWnd(m_hwnd);
	g_db.SetLogPtr(m_report);
	g_pManager->setLogPtr(m_report);
	/*开启远程任务处理线程*/
	g_pManager->startHandleRemoteTask();
	/*初始化服务部分*/
	...
	/*初始化数据库*/
	m_ret = g_db.Open(DB_HOST, DB_PORT, DB_USER, DB_PWD, DB_NAME);
	if (!m_ret)
	{
